...
/** The map's lines as the motion reads them. `path` answers a route's
 *  SVG path from one station to another — `to` null for the off-ramp a
 *  packet leaves the map by — or null where the map draws no such line. */
export type RoutePath = (from: string, to: string | null) => string | null;
...
export type Geometry = Readonly<{
  ...
  lineOf?: (from: string, to: string | null) => string | null;
}>;
...
export function admit(
  ...
  const line = (mv.from !== null ? g.lineOf?.(mv.from, mv.to) : null) ?? 'delivery';
  const land = (ms: Motion): Motion =>
    mv.to !== null && to !== null ? { ...ms, pings: pinged(ms.pings, mv.to, to, line, o.now, o.reduced) } : ms;

  // Filed onto the map with nothing linking it to where it came from: it
  // pings where it lands, and nothing travels (design §3, hand-offs).
  if (mv.from === null || from === null) return land(m);
  // Leaving by an off-ramp, or arriving somewhere the map can place.
  if (mv.to !== null && to === null) return m;
  if (o.reduced) return land(m);

  const key = routeKey(mv.from, mv.to);
  const drawn = g.path(mv.from, mv.to);
  const d = drawn ?? (to === null ? offRamp(from) : straight(from, to));
  const walked = walk(pathPoints(d));
  ...
  const dot: Dot = {
    ...
    undeclared: mv.declared === false,
    off: mv.to === null,
    handoff: mv.handoff_from !== null ? from : null,
