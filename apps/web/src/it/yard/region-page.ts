 ...
 *  STATIONS, each drawn at its own depth; a packet stands at every
 *  station whose predicate it matches, and the server's head counts it
 *  once, and only while it is marshalling's (decision 4). The station
 *  reads carry no partition, so the drawing cannot be narrowed here —
 *  it names what it holds beyond the head instead. */
const WHY_THE_PLATFORMS_DIFFER: Readonly<Record<string, string>> = {
  marshalling:
...
/** The sentence an interior owes when it does not draw the head's
 *  count (decision 5: "each interior draws the header's count, or names
 *  what it leaves out"). Null when the two agree, and when either side
 *  is a number nobody could take — the head already says "no reading",
 *  and an unknown platform is drawn as `?`. */
export function drawnNote(r: Region | undefined, platforms: ReadonlyArray<Platform>): string | null {
  if (r === undefined || r.count === null) return null;
  if (platforms.some((p) => p.standing === null)) return null;
  const drawn = platforms.reduce((n, p) => n + (p.standing ?? 0), 0);
  if (drawn === r.count) return null;
  const unit = r.unit === '' ? '' : ` ${r.unit}`;
  const why = WHY_THE_PLATFORMS_DIFFER[r.name];
  return `the platforms stand ${drawn} — the head counts ${r.count}${unit}${why ? `: ${why}` : ''}`;
}
