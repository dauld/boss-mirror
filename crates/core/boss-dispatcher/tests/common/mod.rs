...
//!
//! Provides:
//! - `RULES_DIR` — the authored directory, relative to this crate
...
pub fn authored_rule(name: &str) -> Registry {
    ...
    let toml =
        std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("read rule file {path}: {e}"));
    Registry::from_toml(&toml).unwrap_or_else(|e| panic!("rule file {path} parses: {e}"))
}
