...
pub struct StationReading {
    ...
    /// When the oldest packet standing here opened — the marshalling
    /// KPI (design 62de32ae, decision 9). `None` when nothing stands or
    /// no member carries an opening the record holds.
...
fn receiving(inputs: &RegionInputs<'_>, w: &Windows) -> Region {
    ...
    // unrecorded — is car B's (backlog c3105b2a): a share over this
    // read, which floors at its page, would be a guess, and B is the car
    // that reads past the page.
    #[allow(clippy::cast_precision_loss)]
...
fn marshalling(inputs: &RegionInputs<'_>, w: &Windows) -> Region {
    ...
    let Some(stations) = inputs.stations else {
        return region(
            ...
            None,
            UNIT,
            unread_settled("the station registry could not be read", inputs.now),
            Trend {
    ...
    // hours from its opening.
    let oldest = stations
        .iter()
        .filter_map(|s| s.oldest)
        .min()
...
fn stations_stuck(inputs: &RegionInputs<'_>) -> StuckPart {
    let Some(stations) = inputs.stations else {
        return StuckPart::unread(
            "marshalling",
            "marshalling: the station registry could not be read",
        );
    };
    let stuck = stations
        .iter()
    ...
    fn marshalling_counts_packets_once_and_a_station_not_draining_is_trouble() {
        let stations = vec![
            StationReading {
                ...
                oldest: Some(t("2026-09-18T12:00:00Z")),
            },
            StationReading {
                ...
                oldest: Some(t("2026-09-19T06:00:00Z")),
            },
            StationReading {
