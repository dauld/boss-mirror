...
async fn run_server<R: AssetsRepository + 'static>(
    ...
    let state = AssetsApiState {
        ...
        hub,
        // Asset-event gate (Update on asset). Until 2026-09-26 this was
        // `policy: None`, so the gate in http.rs skipped and any caller
        // reaching the port could append asset events (backlog
        // 54bf2e1e). Same wrapping as people and ledger: on a sim
        // instance a sim caller is authorized at the boundary;
