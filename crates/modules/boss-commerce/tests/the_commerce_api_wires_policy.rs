//! The production binary builds the commerce surface WITH a policy
//! client (backlog 54bf2e1e, 2026-09-26).
//!
//! `None` is the test path — the invoice-create gate in `http.rs` is
//! `if let Some(ref policy) = state.policy`, so it allows — and the
//! binary passed it, so the Create-on-invoice check never ran in
//! production. The same defect boss-people had until 8cdad84c, pinned
//! the same way (`the_people_api_wires_policy.rs`): this reads the
...
#[test]
fn the_commerce_api_binary_never_builds_its_state_without_policy() {
    ...
    assert!(
        unwired.is_empty(),
        "boss_commerce_api.rs builds its state with no policy: {unwired:?}"
    );
