...
method_json() {
    jq -n '{
      ...
      fields: "label, description, category — the scalar strings an operator reads, compared between each infra/platform/workflows/<kind>.toml and the ACTIVE live row of that kind. Structural fields (steps, subject_kinds, metadata_schema, entitlements) are deliberately not compared: a live row legitimately leads its file between a publish and the car that writes it down, and comparing them needs the normalisation the publish path applies first.",
